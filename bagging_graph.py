...
for i, updrs in enumerate(updrs_list):
    ...
    for j in range (50, estimators[i] + 1, 50):
        model = BaggingClassifier(estimator=DecisionTreeClassifier(max_depth = depth[i]), n_estimators=j)
        model.fit(X_train, Y_train)
        y_pred = model.predict(X_test)
        mse = mean_squared_error(Y_test, y_pred)
        mse_per_it.append(mse)
    axs[i].set_title(updrs)
    axs[i].plot(x_range[:(estimators[i]//50)], mse_per_it, label = updrs)
